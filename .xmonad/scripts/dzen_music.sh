...
art="/home/daniel/.config/ario/covers/$(ls ~/.config/ario/covers | grep SMALL | grep "$(mpc current -f %album%)")"
...
(echo "^fg($highlight)Music"; echo "^ca(1,/home/daniel/.xmonad/scripts/dzen_lyrics.sh) $track^ca()"; echo " ^ca(1,/home/daniel/.xmonad/scripts/dzen_artistinfo.sh)^fg()$artist^ca()";echo " ^ca(1,/home/daniel/.xmonad/scripts/dzen_albuminfo.sh)^fg()$album^ca()"; echo " "; echo "        ^ca(1, ncmpcpp prev)  ^fg($white0)^i(/home/daniel/.xmonad/dzen2/prev.xbm) ^ca()  ^ca(1, ncmpcpp pause) ^i(/home/daniel/.xmonad/dzen2/pause.xbm) ^ca()  ^ca(1, ncmpcpp play) ^i(/home/daniel/.xmonad/dzen2/play.xbm) ^ca()   ^ca(1, ncmpcpp next) ^i(/home/daniel/.xmonad/dzen2/next.xbm) ^ca()"; echo " "; echo $percbar; sleep 15) | dzen2 -fg $foreground -bg $background -fn $FONT -x $XPOS -y $YPOS -w $WIDTH -l $LINES -e 'onstart=uncollapse,hide;button1=exit;button3=exit' & 
sleep 15
killall feh
